...
离线 schema v1 aggregation 合约与实现位于 [`aggregation.md`](aggregation.md) / `research/stage2_aggregate.py`；独立分支已通过 Mac 对抗审查并进入 Draft [PR #20](https://github.com/NEVER-AGAIN-RAY/NanoServeLab/pull/20)。本切片未在正式 raw 上运行汇总，也没有性能结论。下一门槛是审阅并合并 aggregation 后，再对三份正式 raw 做只读汇总。
